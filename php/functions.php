...
add_action( 'vc_backend_editor_enqueue_js_css', 'codeus_vc_backend_editor_enqueue_js_css' );
function codeus_vc_backend_editor_enqueue_js_css () {
	wp_enqueue_script('codeus_js_composer_js_custom_views', get_template_directory_uri() . '/js/codeus-composer-custom-views.js', array(), '', true );
}
...
function codeus_load_fonts() {
	...
	foreach($fontElements as $element) {
		if(($font = codeus_get_option($element.'_family')) && !in_array($font, $exclude_array) && isset($fontsList[$font])) {
			...
			wp_enqueue_style($font['family'].'-'.$font['variants'].'-'.$font['subsets'],'//fonts.googleapis.com/css?family='.$font['family'].':'.$font['variants'].'&subset='.$font['subsets']);
		}
	}
}
...
?>
	<?php if($total > 1) : ?>
		<div class="pagination">
			...
			<?php endif; ?>
			<?php $hidden = false; for($i=0; $i < $total; $i++) : ?>
				<?php if($i == 0 || $i == $total - 1 || ($i > $current - 4 && $i < $current + 2 )) : $hidden = false; ?>
					<?php if($current != $i+1) : ?>
						<a href="<?php echo get_pagenum_link($i+1); ?>"><?php echo $i+1; ?></a>
					<?php else : ?>
						<span class="current"><?php echo $i+1; ?></span>
					<?php endif; ?>
				<?php elseif(!$hidden) : $hidden = true; ?>
...
function codeus_woocommerce_account_menu_item_classes($classes, $endpoint) {
	if(in_array('is-active', $classes)) {
		$classes[] = 'current-menu-item';
	}
	return $classes;
}
add_filter('woocommerce_account_menu_item_classes', 'codeus_woocommerce_account_menu_item_classes', 10, 2);

function bb_styles_and_scripts() {
	wp_enqueue_style( 'bootstrap', get_template_directory_uri() . '/bb/css/bootstrap.min.css');
	wp_enqueue_style( 'animate', get_template_directory_uri() . '/bb/css/animate.css');
	wp_enqueue_style( 'style', get_template_directory_uri() . '/bb/css/style.css');
	wp_enqueue_style( 'bbmedia', get_template_directory_uri() . '/bb/css/bbmedia.css');
	
	//wp_enqueue_script( 'bbjquery', get_template_directory_uri() . '/bb/js/bbjquery.js');
	wp_enqueue_script( 'bootstrap', get_template_directory_uri() . '/bb/js/bootstrap.min.js');
	wp_enqueue_script( 'viewportchecker', get_template_directory_uri() . '/bb/js/viewportchecker.min.js');
	wp_enqueue_script( 'bbcommon', get_template_directory_uri() . '/bb/js/bbcommon.js');
}

add_action( 'bb-head', 'bb_styles_and_scripts' );
